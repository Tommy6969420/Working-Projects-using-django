...
function openChat(to) {
  resetBg()
  document.getElementById(`active-message`).innerHTML = document.getElementById(`txt${to}`).innerHTML;
  console.log(to)
  msg_div.innerHTML = ""
    ...
    .then(data => {
      ...
      if (data.length >= 1) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          txts = data[`${i}`]["message"]
          id = data[`${i}`]["id"]
          msg_div.innerHTML += `<div class="message reciever" id="text.1-${id}">
          ${txts}
        </div> `
    ...
    .then(data => {
      ...
      if (data.length >= 1) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          txts = data[`${i}`]["message"]
          id = data[`${i}`]["id"]
          msg_div.innerHTML += `       <div class="message sender" id="text.1-${id}">
          ${txts}
        </div> `
          msg_div.scrollTop = msg_div.scrollHeight;
...
function sortMsg() {
  var toSort = document.getElementById('chat').children;
